...
steps:
	...
	@echo ""
	@echo "  2. Istio ambient (helm — order matters: base → istiod → cni → ztunnel)"
	@echo "     helm repo add istio https://istio-release.storage.googleapis.com/charts"
	@echo "     helm repo update istio"
	@echo "     helm install istio-base istio/base -n istio-system --create-namespace --wait"
	@echo "     helm install istiod istio/istiod -n istio-system --set profile=ambient \\"
	@echo "       -f deploy/00-operators/helm/istio-ambient-values.yaml --wait"
	@echo "     helm install istio-cni istio/cni -n istio-system --set profile=ambient --wait"
	@echo "     helm install ztunnel istio/ztunnel -n istio-system --wait"
	@echo "     kubectl apply -f deploy/00-operators/ambient-ztunnel.yaml   # cluster-default Telemetry CR"
	@echo ""
	@echo "  3. Kgateway (helm) — CRDs + controller with waypoint subchart enabled"
	@echo "     helm upgrade -i --create-namespace --namespace kgateway-system --version v2.1.0 \\"
	@echo "       kgateway-crds oci://cr.kgateway.dev/kgateway-dev/charts/kgateway-crds \\"
	@echo "       --set controller.image.pullPolicy=Always"
	@echo "     helm upgrade -i --namespace kgateway-system --version v2.1.0 \\"
	@echo "       kgateway oci://cr.kgateway.dev/kgateway-dev/charts/kgateway \\"
	@echo "       --set controller.image.pullPolicy=Always \\"
	@echo "       --set agentgateway.enabled=true \\"
	@echo "       --set waypoint.enabled=true"
	@echo "     kubectl apply -f deploy/10-mesh/kgateway.yaml"
	@echo ""
	@echo "  4. Waypoint for the otel-demo namespace (uses kgateway-waypoint GatewayClass)"
